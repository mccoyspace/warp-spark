...
def _validate_cuda_arms(cpu: Capture, gpu: Capture) -> dict[str, Any] | None:
    ...
    if cpu.arm.key == "cuda_vq":
        counter_names = ("experts", "applies", "lut_builds", "launches", "syncs")
        if cpu.arm.value == 2 or gpu.arm.value == 3:
            if cpu.arm.value != 2 or gpu.arm.value != 3:
                raise CaptureError(
                    "CUDA VQ fused qualification requires a mode-2 control "
                    "and mode-3 candidate"
                )
            decode_steps = len(gpu.steps) - 1
            if decode_steps <= 0 or len(cpu.steps) != len(gpu.steps):
                raise CaptureError(
                    "CUDA VQ fused captures require matched decode steps"
                )
            expected_gqa = 368 * decode_steps
            expected_dense = 644 * decode_steps
            route_work = None
            for label, capture, mode in (
                    ("control", cpu, 2), ("candidate", gpu, 3)):
                arm = capture.arm
                assert arm is not None
                if (not _valid_kda_base(arm, (1,)) or
                        arm.kda_expected_calls != 0 or
                        arm.dense_scope != 3 or arm.dense_effective != 3 or
                        arm.dense_calls != expected_dense or
                        arm.dense_expected_calls != expected_dense or
                        arm.gqa_proj != 1 or arm.gqa_proj_effective != 1 or
                        arm.gqa_proj_calls != expected_gqa or
                        arm.gqa_proj_expected_calls != expected_gqa or
                        arm.vq_group != 1 or arm.fallbacks != 0):
                    raise CaptureError(
                        f"CUDA VQ fused {label} lacks the exact "
                        "KDA1/dense3/GQA1/group1 full-GLM base"
                    )
                if (arm.value != mode or arm.effective != mode or
                        arm.vq_mode != mode or arm.vq_effective != mode or
                        arm.calls != arm.vq_launches or
                        arm.expected_calls != arm.vq_expected_launches):
                    raise CaptureError(
                        f"CUDA VQ fused {label} generic and VQ metadata disagree"
                    )
                layer_runs, experts = _vq_route_work(capture)
                if layer_runs == 0:
                    raise CaptureError(
                        f"CUDA VQ fused {label} has no routed workload"
                    )
                current_work = (layer_runs, experts)
                if route_work is not None and current_work != route_work:
                    raise CaptureError(
                        "CUDA VQ fused captures use different routed workloads"
                    )
                route_work = current_work
                expected = {
                    "experts": experts,
                    "applies": 3 * experts,
                    "lut_builds": experts + 2 * layer_runs,
                    "launches": ((3 if mode == 2 else 4) * experts +
                                  layer_runs),
                    "syncs": (2 if mode == 2 else 1) * experts,
                }
                for counter, required in expected.items():
                    actual = getattr(arm, f"vq_{counter}")
                    declared = getattr(arm, f"vq_expected_{counter}")
                    if actual != required or declared != required:
                        raise CaptureError(
                            f"CUDA VQ fused {label} {counter}="
                            f"{actual}/{declared}; route rows require {required}"
                        )
            return {"cpu": _arm_dict(cpu.arm), "gpu": _arm_dict(gpu.arm)}
