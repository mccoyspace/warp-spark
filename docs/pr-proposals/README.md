...
independent so a maintainer can review or land them separately.  Rebase every
branch that is not first to land, rerun its acceptance set, and resolve public
status-code or struct-layout additions against the new base before opening the
next PR.
...
| In-memory state snapshots | `pr/in-memory-state-snapshots` | Pending implementation/acceptance | In progress |
